...
for i=1:1:(ny+nx-1)-ny
    y(i)=0;
end

nnx=input('Enter elements of signal 1\n');
xo=input('Enter origin position to first signal ');
nny=input('\nEnter elements of signal 2\n');
yo=input('Enter origin position to second signal ');
...
display(t);
display('Siganl lower and upper limits are');
nal=al;
aldi=num2str(al*-1);
audi=num2str(au);
nnal=[aldi ' ' audi];
display(nnal);
display('Signal origin is at position');
display(abs(nal)+1);
